"""
sparse_gp/evaluate.py
Run SparseStraddleGPR on a synthetic map and report results.

Outputs (written to --out-dir, default sparse_gp/output/)
----------------------------------------------------------
  truth.png          ground truth binary map
  predicted.png      final predicted binary map
  accuracy_curve.png accuracy vs samples (every refit_interval steps)
...
-----
    ...
    python sparse_gp/evaluate.py --blob_size 0.4 --n_holes 3 --hole_size 6 \
...
from sparse_gp.sparse_gp import SparseStraddleGPR
...
def _save_summary(
    ...
    ax_p.imshow(predicted, cmap="gray", vmin=0, vmax=1, aspect="auto",
                interpolation="nearest")
    ax_p.set_title(f"SparseStraddleGPR Prediction  (acc {accuracy:.2%})", fontsize=10)
    ax_p.axis("off")
    ...
    fig.suptitle("SparseStraddleGPR — Summary", fontsize=12, fontweight="bold")
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
...
def _save_accuracy_curve(
    ...
    ax.set_title("Accuracy vs samples  (SparseStraddleGPR)")
...
def run(
    ...
    _save_binary_png(predicted, out / "predicted.png",
                     f"SparseStraddleGPR prediction  (acc {accuracy:.2%})")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate SparseStraddleGPR on synthetic map."
    )
    ...
    p.add_argument("--n_inducing",     type=int,   default=200)
    p.add_argument("--out-dir",        type=str,   default="sparse_gp/output",
                   dest="out_dir")
    return p.parse_args()

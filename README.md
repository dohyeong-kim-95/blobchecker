...
하나의 2D 좌표 그리드 위에 있는 16개 이진 blob 레이어를 가능한 적은
좌표 질의로 재구성합니다.

한 iteration은 정확히 하나의 좌표 `(row, col)`를 선택합니다. Oracle은 그
좌표에서 16개 레이어의 이진 레이블을 동시에 반환합니다. 알고리즘은 질의
전에 그리드 크기만 알고 시작합니다.
...
| 채점 대상 | `truth_blob_mask` |
| 필수 출력 | `predicted_blob_mask.shape == (16, H, W)` |
| 정확도 기준 | 모든 레이어 `>= 98%` |
...
현재 Shmoo 도메인에는 outlier가 없어 `truth_full_mask == truth_blob_mask`
입니다. 텐서 3종 시그니처는 하위 호환을 위해 유지하며, 향후 outlier를
다시 도입하면 oracle을 통해 관측되지만 핵심 복원 대상은 아닙니다.

## 현재 구현

| 컴포넌트 | 파일 |
|---|---|
| 데이터 생성기 (레거시 가우시안, 교체 예정) | `src/generator.py` |
| 데이터 생성기 (Shmoo, 정본 설계) | [docs/shmoo_model.md](docs/shmoo_model.md) — 미구현 |
...
## 최신 결과 (피벗 이전 — 레거시 가우시안 8레이어 / 50 x 200)

아래 결과는 도메인 피벗 이전 가우시안 생성기·8레이어·50 x 200 기준입니다.
Shmoo 생성기(16레이어·150 x 200) 구현 후 재측정해야 합니다.
...
| [docs/shmoo_model.md](docs/shmoo_model.md) | Shmoo 데이터 생성기 정본 설계입니다. |
| [todo.md](todo.md) | Shmoo 생성기 원본 요구 프롬프트입니다. |
